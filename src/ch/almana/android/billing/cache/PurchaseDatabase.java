...
public class PurchaseDatabase {
	...
	PurchaseDatabase(Context context) {
...
//        values.put(HISTORY_ORDER_ID_COL, orderId);
    ...
    private void updatePurchasedItem(String productId, int quantity) {
		...
		long ret = mDb.replace(PURCHASED_ITEMS_TABLE_NAME, null /* nullColumnHack */, values);
    }
	 ...
	 */
	synchronized int updatePurchase(/*String orderId,*/String productId,
     ...
     */
	Cursor getAllPurchasedItemsCursor() {
	...
	Map<String, Product> getOwnedItems() {
		Cursor cursor = getAllPurchasedItemsCursor();
		final Map<String, Product> ownedItems = new HashMap<String, Product>();
		if (cursor == null) {
			return ownedItems;
		}
		...
		try {
			...
			int productCountCol = cursor.getColumnIndexOrThrow(PurchaseDatabase.PURCHASED_QUANTITY_COL);
			while (cursor.moveToNext()) {
				String productId = cursor.getString(productIdCol);
				int productCount = (int) cursor.getLong(productCountCol);
				ownedItems.put(productId, new Product(productId, productCount));
			}
		} finally {
			cursor.close();
		}
