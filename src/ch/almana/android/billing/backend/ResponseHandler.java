...
public class ResponseHandler {
     ...
     */
	private static List<PurchaseObserver> purchaseObservers = new ArrayList<PurchaseObserver>();
    ...
    public static synchronized void register(PurchaseObserver observer) {
		purchaseObservers.add(observer);
    }
    ...
    public static synchronized void unregister(PurchaseObserver observer) {
		purchaseObservers.remove(observer);
    }
    ...
    public static void checkBillingSupportedResponse(boolean supported) {
		for (PurchaseObserver observer : purchaseObservers) {
			observer.onBillingSupported(supported);
		}
    }
    ...
    public static void purchaseResponse(
		...
		// // value of sPurchaseObserver.
		 synchronized(ResponseHandler.class) {
	    	for (PurchaseObserver observer : purchaseObservers) {
				observer.postPurchaseStateChange(purchaseState, productId, purchaseTime, developerPayload);
			}
		 }
    }
    ...
    public static void responseCodeReceived(Context context, RequestPurchase request,
    	...
    	for (PurchaseObserver observer : purchaseObservers) {
			observer.onRequestPurchaseResponse(request, responseCode);
		}
    }
    ...
    public static void responseCodeReceived(Context context, RestoreTransactions request,
            ResponseCode responseCode) {
    	for (PurchaseObserver observer : purchaseObservers) {
			observer.onRestoreTransactionsResponse(request, responseCode);
		}
    }
}
